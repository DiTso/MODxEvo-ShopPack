...
1) скачиваем отсюда http://simplehtmldom.sourceforge.net/  библиотеку simple_html_dom и кидаеи ее в папочку /assets/lib/  нам нужен тольк один файлик — simple_html_dom.php

2)Создаем страницу с exit.html (переход на внешнюю ссылку) c вызовом сниппета [!exturl!]  с кодом return $_GET['url'];
...
события  
OnWebPagePrerender, OnPageNotFound


//Автор Алексей Либер alexey@liber.pro
$error_page = 'exit.html'; //Уникальное название страницы, куда первоначально будем делать редирект при внешнем ресурсе 
        
$e =&$modx->Event;
 if (!isset($e->params['compressed'])){$e->params['compressed'] = 'true';}  
